...
uint32_t SpriteRendererGpu::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags props) const {
...
bool endOneTimeCommands(VkDevice device, VkCommandPool pool,
...
bool SpriteRendererGpu::init(VulkanContext* ctx,
    ...
    m_ctx = ctx;
    if (!createTextureImage(pixels, width, height) || !createDescriptorSet(layout)) {
        cleanup();
        return false;
    }
...
bool SpriteRendererGpu::createTextureImage(const std::vector<unsigned char>& pixels,
                                           uint32_t width,
                                           uint32_t height) {
    VkDevice device = m_ctx->device();
