...
VulkanFrameCapture::~VulkanFrameCapture() {
    cleanup();
}
...
bool VulkanFrameCapture::prepare(VkExtent2D extent, VkFormat format) {
    if (!m_context || extent.width == 0 || extent.height == 0 || !supports(format)) {
        return false;
    }

    const VkDeviceSize size = static_cast<VkDeviceSize>(extent.width) *
                              static_cast<VkDeviceSize>(extent.height) * 4u;
    if (m_stagingBuffer && m_stagingSize >= size) return true;
    return recreateBuffer(size);
}

bool VulkanFrameCapture::record(VkCommandBuffer commandBuffer, VkImage image,
                                VkExtent2D extent, VkFormat format) {
    if (!commandBuffer || image == VK_NULL_HANDLE || !prepare(extent, format)) return false;
...
bool VulkanFrameCapture::writePpm(const std::string& path, VkFormat format,
                                  VkExtent2D extent) {
    if (!m_context || !m_stagingMemory || !supports(format) ||
        extent.width == 0 || extent.height == 0) return false;
    ...
    void* mapped = nullptr;
    if (vkMapMemory(m_context->device(), m_stagingMemory, 0, required, 0, &mapped) != VK_SUCCESS) {
        return false;
    }
    ...
    for (uint32_t y = 0; y < extent.height; ++y) {
        ...
        for (uint32_t x = 0; x < extent.width; ++x) {
            const size_t index =
                (static_cast<size_t>(sourceY) * extent.width + x) * 4u;
