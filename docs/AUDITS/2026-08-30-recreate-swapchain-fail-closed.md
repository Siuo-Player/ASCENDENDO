...
The current ASCENDENDO code follows the same fail-closed structure. The current window is non-resizable, so a synthetic zero-extent resize path is not a legitimate current test mechanism.

## Current property

A dependent failure reached after a successful `vkDeviceWaitIdle()` causes `RendererCore::recreateSwapchain()` to return `false`, leave `m_initialized == false`, and reject a subsequent recreation attempt.

The focused test exercises this through the existing public API: it waits for the device to become idle, cleans the `Swapchain`, then invokes `RendererCore::recreateSwapchain()`. `Swapchain::recreate()` rejects the now-uninitialized dependency, so the core remains inactive.
...
RendererCore inactive
      ↓
drawFrame() cannot continue through the initialized path
      ↓
