...
ASCENDENDO é um **Vertical Precision Platformer** construído sobre um motor 2D próprio. O objetivo não é apenas entregar um jogo com alguns níveis: é criar uma experiência de plataforma vertical precisa e um sistema de autoria suficientemente fiável para que novos níveis possam continuar a ser criados, testados, validados, jogados e partilhados durante muitos anos.
...
O trabalho 9.6 actual já tem a base de modelo, persistência e validação. O próximo passo estrutural é integrar esse modelo na experiência visual do editor.
