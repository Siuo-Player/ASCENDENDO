...
A força apresentada ao jogador deve representar directamente o estado real da simulação. Não existe UI `0–255` ou outra escala artificial de precisão.
...
A arquitectura continua aberta a perigos, moving platforms, triggers, colectáveis, checkpoints e outros objectivos futuros, mas essas mecânicas não entram automaticamente em 1.0.

## 5. Transição e streaming

A passagem entre screens é contínua. O jogo deve manter as zonas necessárias para uma transição suave e nunca deve introduzir uma pausa visível nem alterar o resultado determinístico da simulação.

## 6. Level Editor

O Level Editor edita o level completo e preserva a screen `640×360` como unidade de leitura.
...
Cada campanha progride aproximadamente do fácil para o difícil/extremo. A escala nominal de dificuldade deve derivar do modelo e da validação reais, não de um número arbitrário congelado.
...
O replay é reexecutado pelo motor. O tempo produzido pela execução válida é a medida autoritativa da run.

## 11. Visual

Direcção 1.0: **indie, simples, pixel-art básica**, com gameplay readability acima de decoração.
...
- largura **e altura** da plataforma devem ser múltiplas positivas de `16 px`;
- a composição visual usa uma malha `16×16` local à região;
- o renderer escolhe automaticamente sprites de acordo com adjacência, topologia, material, superfícies expostas, interiores, bordas, cantos e joins;
- regiões/plataformas de materiais diferentes podem compor-se visualmente quando existe contacto geométrico real;
- relva ou outros tratamentos de borda não aparecem em posições onde o contexto visual os torna incoerentes;
- variantes de pixel art podem ser escolhidas com aleatoriedade controlada e determinística para reduzir repetição;
- falta de uma variante compatível produz fallback determinístico;
- auto-tiling é **presentation-only**: nunca altera posição, tamanho, colisão ou outra geometria de gameplay.

`16×16` é uma unidade de composição visual, não uma unidade obrigatória de autoria do mundo.
...
1.0 inclui música retro livre para redistribuição e efeitos de jogo/editor. Variação pseudo-aleatória controlada pode ser usada em efeitos não-musicais; música permanece estável.
...
A arquitectura deve favorecer mods, expansões e jogos derivados futuros sem introduzir em 1.0 mecânicas sem razão de design.
