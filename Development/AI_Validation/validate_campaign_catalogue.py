...
def validate_campaign_catalogue(campaign_file: Path, levels_root: Path) -> list[str]:
    ...
    for line_number, raw in enumerate(lines, 1):
        ...
        if not line or line.startswith("#"):
            continue
        if line != line.strip():
            errors.append(f"line {line_number}: leading/trailing whitespace is not allowed: {line!r}")
            continue
    ...
    for line_number, entry in entries:
        path = Path(entry)
        if path.is_absolute() or any(part == ".." for part in path.parts) or "\\" in entry:
            errors.append(f"line {line_number}: campaign path must be repository-relative and portable: {entry!r}")
            continue
        if path.suffix.lower() != ".lvl":
            errors.append(f"line {line_number}: campaign entry must end in .lvl: {entry!r}")
            continue

        key = path.as_posix().casefold()
        previous = seen.get(key)
        if previous is not None:
            errors.append(f"line {line_number}: duplicate campaign level {entry!r}; first declared on line {previous}")
        else:
