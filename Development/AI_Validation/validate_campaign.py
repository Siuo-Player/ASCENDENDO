...
# The score samples a real control-space instead of selecting one optimal jump.
# 13 x 21 x 2 = 546 simulations per source/target edge is dense enough to expose
# narrow landing/charge windows while keeping campaign CI practical.
CHARGE_SAMPLES = 21
LAUNCH_X_SAMPLES = 13
...
def landing_success(source: Surface, target: Surface, start_x: float, direction: int, charge: float) -> bool:
    landed, fx, fy, _, _ = simulate_jump(start_x, source.top, direction, charge, [rect(source), rect(target)])
...
def landing_profile(source: Surface, target: Surface) -> dict[str, float | int | str] | None:
    """Measure the usable control-space for landing on one target."""
    positions = launch_positions(source)
    usable_positions = 0
    charge_success_sum = 0.0
    best_overall: tuple[float, int, float] | None = None
    for start_x in positions:
        best_direction_successes = 0
        best_direction = 0
        for direction in (-1, 1):
            ...
            if successes > best_direction_successes:
                best_direction_successes = successes
                best_direction = direction
        if best_direction_successes:
            usable_positions += 1
            charge_fraction = best_direction_successes / CHARGE_SAMPLES
            charge_success_sum += charge_fraction
            candidate = (charge_fraction, best_direction, start_x)
    ...
    best_charge = next(
        ...
        for index in range(CHARGE_SAMPLES)
        if landing_success(source, target, best_overall[2], int(best_overall[1]), index / (CHARGE_SAMPLES - 1))
    )
    return {
        ...
        "best_charge": round(best_charge, 3),
        "horizontal_displacement_px": abs(target.x + target.w * 0.5 - (best_overall[2] + PLAYER_WIDTH * 0.5)),
...
def flag_profile(source: Surface, flag: Surface, level: Level) -> dict[str, float | int | str] | None:
    ...
    for start_x in positions:
        ...
        for direction in (-1, 1):
            ...
            for index in range(CHARGE_SAMPLES):
                ...
                hit, _, _ = simulate_jump_flag(start_x, source.top, direction, charge, platforms, rect(flag))
                successes += int(hit)
            direction_best = max(direction_best, successes)
            if successes > direction_best:
                best_direction = direction
        if direction_best:
            usable_positions += 1
            charge_success_sum += direction_best / CHARGE_SAMPLES
            best_window = max(best_window, direction_best / CHARGE_SAMPLES)
    ...
    return {
        ...
        "best_charge_window": best_window,
...
def choose_route(level: Level, final: bool) -> tuple[list[dict[str, object]], float, str]:
    ...
    while heap:
        ...
        for target_index, profile in graph[source_index]:
            next_cost = cost - math.log(float(profile["control_success_probability"]))
    ...
    return transitions, math.exp(-total_cost), "flag_reached" if terminal_profile is not None else "top_reached"
